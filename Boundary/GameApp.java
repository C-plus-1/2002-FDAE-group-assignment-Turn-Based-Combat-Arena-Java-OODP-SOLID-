                ...
                public void onPlayerTurn(Combatant playerTurn, List<Combatant> enemiesTurn) {
                    ...
                    if (actionIndex == 0) {
                        ...
                        if (playerTurn instanceof Warrior) {
                            ...
                            if (playerTurn.isPowerStoneReady()) {
                                playerTurn.setPowerStoneReady(false);
                                chosenAction = new UseSpecialSkill(skill, true);
                            } else {
                                chosenAction = new UseSpecialSkill(skill, false);
                            }
                            needsTarget = true;
                        } else {
                        ...
                        chosenAction = new Action() {
                            ...
                            public void execute(Combatant actor, Combatant target, BattleContext context) {
                                if (selectedItem instanceof SmokeBomb) {
                                    for (Combatant enemy : context.getEnemies()) {
                                        if (enemy.isAlive()) selectedItem.use(enemy);
                                    }
                                } else if (selectedItem instanceof PowerStone) {
                                    SpecialSkill skill = (actor instanceof Warrior) ? new ShieldBash() : new ArcaneBlast();
                                    skill.execute(actor, target, context, true);
                                } else {
                        ...
                        if (selectedItem instanceof PowerStone) {
                            needsTarget = (player instanceof Warrior);
                        } else {
                            needsTarget = false;
                        }
                    }
